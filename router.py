...
class Router(BaseRouter):
    ...
    def inject_dependency(
        ...
        """
        Inject a dependency for a specific route.
        """
        def decorator(handler):
            self.dependencies[endpoint] = dependency
            return self.add_route(route_type, endpoint, handler, is_const, exception_handler)

        return decorator
    def get_injected_dependencies(self) -> Dict[str, Callable]:
        """
        Get the injected dependencies for each route.
        """
